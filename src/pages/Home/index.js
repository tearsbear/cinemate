    ...
    return (
        <div>
            ...
            <div className="search">
                <input placeholder="Search movie ...." onChange={(e) => searchMovie(e.target.value)} />
            </div>

            <div className="movie-list">
                <h1 className="text-white spartan-medium">List Movie</h1>
                {notFound ? <div className="mt-4 text-white pb-5">no movie found ...</div> : ""}

                {!notFound && movieList.map((item, index) =>
                (
                    <div className="movie-card" key={index}>
                        <div className="row">
                            ...
                            <div className="col-8">
                                ...
                                <Link to={`/detail/${item?.imdbID}`} className="btn spartan-medium btn-view">View more</Link>
                            </div>
                        </div>
            ...
            {!notFound && loading ? <div className="text-center text-white pb-5">loading data ...</div> : ""}

            {displayModal && (
                <div
                    className="modalImg"
                >
                    <span class="close" onClick={() => showPoster()}>&times;</span>
